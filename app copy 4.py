...
def prep_sov(df, campaign_col):
    ...
    sov = (ser.to_frame(name=campaign_col).groupby(campaign_col, dropna=True)
             ...
             .sort_values("Posts", ascending=False).reset_index(drop=True))
...
def build_timeseries(df, date_col, left_metric_key, right_metric_key, date_min=None, date_max=None):
    ...
    def agg_metric(frame, key):
        if key in {"__count_posts__", "__count_articles__"}:
...
def build_channel_combo_v2(
    ...
    posts = base.groupby("__chan").size().rename("__posts__")

    if left_key in {"__count_posts__","__count_articles__"}:
        left_ser = posts.rename(left_title)
    else:
    ...
    agg = (pd.concat([posts, left_ser, right_ser], axis=1).reindex(idx).fillna(0.0).reset_index())
    agg.rename(columns={"__posts__": "Posts"}, inplace=True)
...
sov_work, total_posts_work = prep_sov(df_work, campaign_col)
...
# Data Social
tot_posts_social = int(len(df_social_metrics))
...
with cS1: st.metric("Total Posts", f"{tot_posts_social:,}")
...
grp_posts = df_work.groupby(campaign_col).size().rename("Total Post")
tbl = grp_posts.to_frame()
...
tbl["SOV Posts%"]      = (tbl["Total Post"]       / tot_post_sum * 100).round(2) if tot_post_sum else 0.0
...
tbl = tbl[["Campaign", "Total Post", "SOV Posts%", "Total Engagement", "SOV Engagement%", "Total Buzz", "SOV Buzz%"]]

st.dataframe(
    tbl.style.format({"SOV Posts%": "{:.2f}", "SOV Engagement%": "{:.2f}", "SOV Buzz%": "{:.2f}"}),
...
top_n = 8
top_posts = topn_generic(tbl.rename(columns={"Total Post":"Value"}),       "Campaign", "Value", top_n)
...
with c_p1: st.plotly_chart(_pie(top_posts, "SOV by Total Post"),       use_container_width=True)
...
if numeric_cols:
    ...
    left_key_social  = "__count_posts__" if left_metric_choice == LEFT_COUNT_ALIAS else left_metric_choice
    ...
    st.info("Time-series dilewati karena tidak ada metrik numerik untuk sumbu kanan (line).")
...
right_default4 = right_choices4.index("Engagement") if "Engagement" in right_choices4 else (right_choices4.index("engagement") if "engagement" in right_choices4 else 0)
...
left_key4   = "__count_posts__" if left_choice4 == LEFT_ALIAS_SOC else left_choice4
...
fig_social_combo, _ = build_channel_combo_v2(
    df_in=df_work_norm[df_work_norm["__chan_norm"].isin(selected_social_channels)],
...
def_idx_pr = 0
for nm in ["PR Value","pr value","pr_value","prvalue"]:
    if nm in right_choices5: def_idx_pr = right_choices5.index(nm); break
...
fig_main_combo, _ = build_channel_combo_v2(
    df_in=df_work_norm[df_work_norm["__chan_norm"].isin(selected_mainstream_channels)],
